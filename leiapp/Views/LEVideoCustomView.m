...
@implementation LEVideoCustomView
...
-(void)startVideoPlay{
    if (!self.videoPlayer) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(moviePlayBackDidFinish:)
                                                     name:kLENotificationApplicationWillResignActive
                                                   object:self.videoPlayer];
        [[NSNotificationCenter defaultCenter] addObserver:self
...
-(UIImage *)coverImage {
    NSURL *url = [NSURL fileURLWithPath:self.videoPlayPath];
