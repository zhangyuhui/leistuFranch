...
- (NSCachedURLResponse *)dataFromRequest:(NSURLRequest *)request {
    ...
    if ([fileManager fileExistsAtPath:filePath]) {
        ...
        if(expire == false){
            NSString* mimeType = [otherInfo valueForKey:@"MIMEType"];
            //需要缓存的：图片、css、js
            if ([self isConnectionAvailable] && (!([mimeType isEqualToString:@"image/png"]||[mimeType isEqualToString:@"application/octet-stream"]))) {
                expire = true;
            }
        }
        if (expire == false) {
